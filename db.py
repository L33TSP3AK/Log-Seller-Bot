...
async def create_tables():
    async with aiosqlite.connect(MAIN_DATABASE_PATH) as db:
        ...
        await c.execute('''CREATE TABLE IF NOT EXISTS logs 
                            ...
                            password TEXT,
                            date TEXT)''')
        await c.execute('''CREATE TABLE IF NOT EXISTS cookies 
                            (id INTEGER PRIMARY KEY AUTOINCREMENT,
                            website TEXT,
                            price INTEGER,
                            path TEXT,
                            date TEXT)''')
        await c.execute('''CREATE TABLE IF NOT EXISTS bl_configs
                            (id INTEGER PRIMARY KEY AUTOINCREMENT,
                            name TEXT,
                            price INTEGER,
                            path TEXT,
...
async def add_purchases(user_id, service, data, amount):
    date = strftime("%Y-%m-%d %H:%M:%S")
    async with aiosqlite.connect(MAIN_DATABASE_PATH) as db:
        c = await db.cursor()
        await c.execute("INSERT INTO purchases VALUES(?,?,?,?,?)",(user_id, service, data, amount, date,))
        await db.commit()
...
async def add_logs(website, price, login, password):
    date = strftime("%Y-%m-%d %H:%M:%S")
    async with aiosqlite.connect(MAIN_DATABASE_PATH) as db:
        c = await db.cursor()
        await c.execute("INSERT INTO logs VALUES(?,?,?,?,?,?)",(None, website, price, login, password,date,))
        await db.commit()

async def add_cookies(website, price, path):
    date = strftime("%Y-%m-%d %H:%M:%S")
    async with aiosqlite.connect(MAIN_DATABASE_PATH) as db:
        c = await db.cursor()
        await c.execute("INSERT INTO cookies VALUES(?,?,?,?,?)",(None, website, price, path, date,))
        await db.commit()

async def add_bl_config(name, price, path):
    date = strftime("%Y-%m-%d %H:%M:%S")
    async with aiosqlite.connect(MAIN_DATABASE_PATH) as db:
        c = await db.cursor()
        await c.execute("INSERT INTO bl_configs VALUES(?,?,?,?,?)",(None, name, price, path, date,))
        await db.commit()